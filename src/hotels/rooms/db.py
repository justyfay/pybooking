...
from sqlalchemy import select, CTE, or_, and_, func, Select, RowMapping, Result
...
class RoomsDb(BaseDb):
    ...
    @classmethod
    async def find_rooms(cls, hotel_id: int, arrival_date: date, departure_date: date) -> Sequence[RowMapping]:
        ...
        :return: object :class:`Sequence[RowMapping]` - список смапленных данных по комнатам
        """
        booked_rooms: CTE = (
        ...
        room_offers: Select[Tuple[Any]] = (
            select(
                ...
                (Rooms.price * (departure_date - arrival_date).days).label("total_cost"),
                (Rooms.quantity - func.coalesce(booked_rooms.c.rooms_booked, 0)).label("room_offers"),
            )
        ...
        async with async_session_maker() as session:
            logger.debug(f"SQL Query: '{room_offers.compile(engine.engine, compile_kwargs={'literal_binds': True})}'")
